  ...
  List<Document_key>& document_path_keys)
{
  ...
  /* This is a little bit trick here. It's just a assumption:
     when field name start with '`'.
     ...
     SEP doc SEP a SEP b SEP c.
   */
  if(field_name[0] != '`')
  {
    buffer.copy("`", 1, &my_charset_bin);
    buffer.append(field_name);
    buffer.append("`");
  }
  else
  {
    buffer.copy(field_name,
                strlen(field_name),
                &my_charset_bin);
  }
  ...
  {
    buffer.append(".`");
    buffer.append(p->string, p->length);
    buffer.append("`");
  }
  buffer.append('\0');
  return strdup_root(mem_root, buffer.c_ptr_safe());
}
...
static enum_field_types field_types_merge_rules [FIELDTYPE_NUM][FIELDTYPE_NUM]=
{
  ...
  //MYSQL_TYPE_NEWDATE      MYSQL_TYPE_VARCHAR
    MYSQL_TYPE_VARCHAR,     MYSQL_TYPE_VARCHAR,
  //MYSQL_TYPE_BIT          <16>-<243>
    MYSQL_TYPE_VARCHAR,
  //MYSQL_TYPE_DOCUMENT     MYSQL_TYPE_DOCUMENT_VALUE
  ...
  //MYSQL_TYPE_BIT          <16>-<243>
    MYSQL_TYPE_BLOB,
  //MYSQL_TYPE_DOCUMENT     MYSQL_TYPE_DOCUMENT_VALUE
    MYSQL_TYPE_DOCUMENT,    MYSQL_TYPE_DOCUMENT,
  //MYSQL_TYPE_NEWDECIMAL   MYSQL_TYPE_ENUM
    MYSQL_TYPE_BLOB,        MYSQL_TYPE_BLOB,
  //MYSQL_TYPE_SET          MYSQL_TYPE_TINY_BLOB
    MYSQL_TYPE_BLOB,        MYSQL_TYPE_BLOB,
...
Field *Field::new_field(MEM_ROOT *root, TABLE *new_table,
                        ...
                        bool from_alias MY_ATTRIBUTE((unused)))
{
...
Field *Field_string::new_field(MEM_ROOT *root,
                               TABLE *new_table,
                               bool keep_type,
                               bool from_alias)
{
  ...
  if (type() != MYSQL_TYPE_VAR_STRING || keep_type)
    field= Field::new_field(root, new_table, keep_type, from_alias);
...
Field *Field_varstring::new_field(MEM_ROOT *root,
                                  TABLE *new_table,
                                  bool keep_type,
                                  bool from_alias)
{
  Field_varstring *res= (Field_varstring*) Field::new_field(root,
                                                            new_table,
                                                            keep_type,
                                                            from_alias);
  if (res)
...
fbson::FbsonErrType pushPathKeys(Document_path_iterator &iter,
...

{
  ...
  while(nullptr != key ){
    ...
    if(updater.getCurrent()->isObject()){
      if(key->string){
        ret = updater.pushPathKey(key->string, key->length);
      }else{
...
inline const fbson::FbsonValue *build_path(
  Document_path_iterator& iter,
  ...
  if(parent_type == fbson::FbsonType::T_Object){
    ...
    writer.writeKey(key->string, key->length);
  }
...
void append_key_path(List<Document_key> &to,
                     List<Document_key> &from,
                     int skip_num = 0)
{
  List_iterator<Document_key> it(from);
  for(Document_key *p; (p=it++);)
  {
    if(skip_num > 0)
    {
...
/* Set the document path keys and number of elements */
void Field_document::set_prefix_document_path(List<Document_key> &pre)
{
  prefix_document_key_path.empty();
  append_key_path(prefix_document_key_path, pre);
  prefix_path_num = pre.elements;
}

Field_document_ref::Field_document_ref(MEM_ROOT *mem_root,
                                       Item_field **inner,
                                       List<Document_key> &doc_path,
                                       int skip_num)
    :Field_document(mem_root,
                    (Field_document*)(*inner)->field,
                    doc_path,
                    skip_num),
     inner_ref(inner)
{
...
Field_document* Field_document_ref::get_inner_field()
{
  DBUG_ASSERT((*inner_ref)->field->type() == MYSQL_TYPE_DOCUMENT);
  return (Field_document*)(*inner_ref)->field;
}

/*
  This function is for checking the number of Document_key
  in the key_list that has already been in the document path.
  If the document path in the field is not a prefix of key_list,
  then return -1. Or return the number of prefix.
 */
int Field_document::document_path_prefix(List<Document_key> &key_list)
{
  List_iterator<Document_key> key_iter(key_list);
  /*
    We use .ref() to get elements so we need to init it.
    In sql_list, only when operator++(int) is called, the
    internal value will be set.
    a = key_iter++;
    b = key_iter.ref();
    and we have a == b.
  */
  key_iter++;
  return document_path_prefix_helper(key_iter);
}

int Field_document::document_path_prefix_helper(
  List_iterator<Document_key> &key_iter)
{
  int count = 0;
  List_iterator<Document_key> it;

  if(nullptr == get_inner_field())
  {
    it.init(prefix_document_key_path);
    it++;
  }
  else
  {
    // Check inner_field and document_key_path
    int res = get_inner_field()->document_path_prefix_helper(key_iter);
    if(res == -1)
    {
      return -1;
    }
    it.init(document_key_path);
    it++;
    count += res;
  }

  while(true){
    Document_key *k1 = *key_iter.ref();
    Document_key *k2 = *it.ref();
    if(k2 == nullptr)
      return count;
...
bool Field_document::eq(Field *field)
{
  if(field->type() != MYSQL_TYPE_DOCUMENT)
    return false;
  if(!Field_blob::eq(((Field_document*)field)->real_field()))
    return false;
  return 0 == my_strcasecmp(system_charset_info,
                            field_name,
                            field->field_name);
}
void Field_document::make_field(Send_field *field){
  Field_blob::make_field(field);
}
  ...
  Save_in_field_args *args)
{
  ...
  /*
    The update arguments maybe null. We set
    default values for them.
   */
  Save_in_field_args::FuncType func_type =
    args ?
    args->func_type :
    Save_in_field_args::FuncType::FUNC_SET;

  Save_in_field_args::CheckType exist_type =
    args ?
    args->exist_type :
    Save_in_field_args::CheckType::CHECK_NONE;

  Save_in_field_args::ArgType arg_type =
    args ?
    args->arg_type :
    Save_in_field_args::ArgType::ARG_SINGLE;
  ...
  if(exist_type == Save_in_field_args::CheckType::CHECK_EXISTS &&
  ...
  if(func_type == Save_in_field_args::FuncType::FUNC_INSERTAT){
    Document_key *key = *iter.ref();
    ...
    if(updater.getCurrent()->isArray() &&
       key->string && key->index >= 0){
      if(Save_in_field_args::ArgType::ARG_ALL == arg_type &&
         val->isArray()){
...
void Field_document::set_null(my_ptrdiff_t row_offset)
{
  if(!is_derived())
    ...
    {
      Document_path_iterator path(this);
      fbson::FbsonValueCreater creater;
      real_field()->
        update_json(creater(), charset(), path, this);
    }
}
                            ...
                            Field_document *from)
{
  ...
  {
    /*
      When prefix_path_num == 0, it means that it's the field defined
      in the original table, where only array and object is allowed to
      be stored.
    */
    if(prefix_path_num == 0 && !val->isArray() && !val->isObject())
    {
      ...
      if(val->isString())
      {
        const char *json = ((fbson::StringVal*)val)->getBlob();
        unsigned int blob_len = ((fbson::StringVal*)val)->getBlobLen();
        unsigned int str_len = ((fbson::StringVal*)val)->length();
        if(blob_len == str_len)
          str_buffer.copy(json, str_len, cs);
        json = str_buffer.c_ptr();
      }else
      {
        json = toJson.json(val);
      }
  ...
  if(TYPE_OK != (pre_ret = prepare_update(cs, &blob)))
  {
    return pre_ret;
  }
...
void Field_document::get_document_path(List<Document_key> &key_path){
  /*
    Here we will always append the document path to key_path from the
    innerest field to the outer ones using recursive.
   */
  if(nullptr != get_inner_field()){
    get_inner_field()->get_document_path(key_path);
  }
  else
  {
    append_key_path(key_path, prefix_document_key_path);
  }
  append_key_path(key_path, document_key_path);
}

Field_document *Field_document::new_field(MEM_ROOT *root,
                                          TABLE *new_table,
                                          bool keep_type,
                                          bool from_alias)
{
  Field_document *new_one = nullptr;
  if(nullptr == get_inner_field())
  {
    new_one = (Field_document*)Field::new_field(root,
                                                new_table,
                                                keep_type,
                                                from_alias);
...
type_conversion_status
Field_document::store_document_value(
  fbson::FbsonValue *val,
  const CHARSET_INFO *cs)
{
  Document_path_iterator path(this);
  return real_field()->
    update_json(val, cs, path, this);
}
...
{
  /*
    This node is the one created in original table. So, it's the one
  ...
  */
     ...
     document_key_path.elements == 0)
  {
    ...
    /*
      For other situation, such as partial update, update the
      column in temp table, it will go through the same logic.
      Scalar value is allowed to be stored.
    */
    fbson::FbsonValueCreater creater;
    Document_path_iterator path(this);
    return real_field()->
      update_json(creater(from, length), cs, path, this);
  }
  return TYPE_ERR_BAD_VALUE;
}
...
{
  /*
    This function can only be called from the field that's
    defined in the table, both table in innodb or temp table.
  */
...
Field_document::set_delete()
{
  ...
  // it must be a doc path
  if(nullptr == update_args){
    return TYPE_ERR_BAD_VALUE;
  }
  Document_path_iterator path(this);
  return real_field()->
    update_json(nullptr, charset(), path, this);
}
...
Field_document::store(longlong nr, bool unsigned_val)
{
  ...
  Document_path_iterator path(this);
  return real_field()->
    update_json(creater((int64_t)nr), charset(), path, this);
}
 ...
 */
static fbson::FbsonValue*
json_extract_fbsonvalue(
  List<Document_key>* key_path_ptr, // in: the key path
  fbson::FbsonValue *pval) // in: fbson value object
{
  ...
  {
    DBUG_ASSERT(key->string != NULL);
    if (pval->isObject())
    {
      /* If pval is an object, this string will be treated as a key,
         even it is a number */
      if (key->string)
        pval = ((fbson::ObjectVal*)pval)->find(key->string);
      else
        pval = nullptr;
    }
...
fbson::FbsonValue *Field_document::val_root_document_value(
  String *val_buffer,
  String *val_ptr)
{
  return real_field()->val_document_value(val_buffer, val_ptr);
}

fbson::FbsonValue *Field_document::val_document_value(
  String *val_buffer,
  String *val_ptr)
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  if(!is_from_index())
  {
    ...
    if(nullptr == fb_val)
      return nullptr;
    val_ptr->set((const char*)fb_val, fb_val->numPackedBytes(), charset());
    return (fbson::FbsonValue*)val_ptr->ptr();
  }
  switch(real_doc_type())
  {
    ...
    case DOC_PATH_INT:
    {
      longlong r = val_int();
      fbson::FbsonValueCreater creater;
      fbson::FbsonValue *fb_val = creater((int64_t)r);
    ...
    case DOC_PATH_DOUBLE:
    {
      double r = val_real();
      fbson::FbsonValueCreater creater;
      fbson::FbsonValue *fb_val = creater(r);
      val_buffer->copy((char*)fb_val, fb_val->numPackedBytes(), charset());
      return (fbson::FbsonValue*)val_buffer->ptr();
    }
    case DOC_PATH_STRING:
    {
      String buffer;
      String *str_ptr;
      str_ptr = val_str(&buffer, &buffer);
      if(nullptr == str_ptr)
        return nullptr;
      fbson::FbsonValueCreater creater;
      fbson::FbsonValue *fb_val = creater(str_ptr->c_ptr(), str_ptr->length());
      val_buffer->copy((char*)fb_val, fb_val->numPackedBytes(), charset());
      return (fbson::FbsonValue*)val_buffer->ptr();
    }
    case DOC_DOCUMENT:
...
template<typename T>
bool extract_val_numeric(
  fbson::FbsonValue *p, T &result, const CHARSET_INFO *cs, TABLE* table){
  result = 0;
  switch (p->type())
  {
    ...
    case fbson::FbsonType::T_Int8:
      result = (T)((fbson::Int8Val*)p)->val();
      break;
    case fbson::FbsonType::T_Int16:
      result = (T)((fbson::Int16Val*)p)->val();
      break;
    case fbson::FbsonType::T_Int32:
      result = (T)((fbson::Int32Val*)p)->val();
      break;
    case fbson::FbsonType::T_Int64:
      result = (T)((fbson::Int64Val*)p)->val();
      break;
    case fbson::FbsonType::T_Double:
      result = (T)((fbson::DoubleVal*)p)->val();
      break;
    case fbson::FbsonType::T_String:
    {
      fbson::StringVal *str = (fbson::StringVal*)p;
      int error;
      char *end;
      uint length = (uint)str->length();
      char *pay_load = (char*)str->getBlob();
      result= my_strntod(cs,
                         pay_load,
                         length,
                         &end,
                         &error);
      if (!table->in_use->no_errors &&
          (error ||
           (length != (uint)(end - pay_load) &&
            !check_if_only_end_space(cs, end, pay_load+length))))
      {
        push_numerical_conversion_warning(current_thd, pay_load,
                                          length, cs,"DOUBLE",
                                          ER_TRUNCATED_WRONG_VALUE);
      }
      break;
...
double Field_document::val_real(void)
{
  ...
  switch(real_doc_type())
  {
    ...
    {
      String str_buff;
      String *res = val_str(&str_buff, &str_buff);
      if(nullptr == res)
        break;
      int error;
      char *end;
      r = my_strntod(charset(),
                     (char*)res->ptr(),
                     res->length(),
                     &end,
                     &error);
      if (!table->in_use->no_errors &&
          (error ||
           (res->length() != (uint)(end - res->ptr()) &&
            !check_if_only_end_space(charset(),
                                     end,
                                     (char*)res->ptr()+res->length()))))
      {
        push_numerical_conversion_warning(current_thd, res->ptr(),
                                          res->length(), charset(), "DOUBLE",
                                          ER_TRUNCATED_WRONG_VALUE);
      }
      break;
    }
...
my_decimal *Field_document::val_decimal(my_decimal *dec){
  ASSERT_COLUMN_MARKED_FOR_READ;
  double ret = this->val_real();
  double2my_decimal(E_DEC_FATAL_ERROR, ret, dec);
  return dec;
}
                                           ...
                                           enum_field_types as_type)
{
  ...
  switch(to_type)
  {
    ...
    default:
    {
      String str_buffer;
      String *res = nullptr;
      res = val_str(&str_buffer, &str_buffer);
      if(nullptr != res)
      {
        uint res_len = std::min(length, (uint)res->length());
        memcpy(buff, res->c_ptr_safe(), res_len);
      }
      break;
    }
  }
                                ...
                                String *val_ptr)
{
  ...
  switch(real_doc_type())
  {
    case DOC_PATH_TINY:
    {
      longlong r = val_int();
      if(r != 0)
        val_buffer->copy("true", strlen("true"), charset());
      else
        val_buffer->copy("false", strlen("false"), charset());
      return val_buffer;
    }
    case DOC_PATH_INT:
    {
      longlong r = val_int();
      char int_buf[128];
      sprintf(int_buf, "%lld", r);
      val_buffer->copy(int_buf, strlen(int_buf), charset());
      return val_buffer;
    }
    case DOC_PATH_DOUBLE:
    {
      double r = val_real();
      char double_buf[128];
      sprintf(double_buf, "%.15g", r);
      val_buffer->copy(double_buf, strlen(double_buf), charset());
      return val_buffer;
    }
    case DOC_PATH_STRING:
    {
      ...
      int len = real->get_length(ptr);
      val_buffer->set(res_ptr, len, charset());
      return val_buffer;
    }
    // If it's not from index, it will be treated as DOC_DOCUMENT.
    case DOC_DOCUMENT:
    {
      String tmp_val_buffer;
      String tmp_val_ptr;
      fbson::FbsonValue *fb_val = val_document_value(
        &tmp_val_buffer,
        &tmp_val_ptr);

      if(nullptr == fb_val || fb_val->isNull())
      {
        val_buffer->set("", 0, charset());
        return nullptr;
      }
      if(fbson::FbsonType::T_String == fb_val->type() ||
         fbson::FbsonType::T_Binary == fb_val->type())
      {
        fbson::StringVal *fb_str = (fbson::StringVal*)fb_val;
        // We don't need to copy the value, because it's in the memory of the
        // document
        if((char*)fb_val == tmp_val_ptr.ptr())
        {
          val_ptr->set(fb_str->getBlob(), fb_str->length(), charset());
          return val_ptr;
        }else
        {
          // It's created.
          val_buffer->copy(fb_str->getBlob(), fb_str->length(), charset());
          return val_buffer;
        }
      }
      // For other cases, we can just convert it to string
      fbson::FbsonToJson toJson;
      const char * json_str = toJson.json(fb_val);
      val_buffer->copy(json_str, strlen(json_str), charset());
      return val_buffer;
    }
    default:
      DBUG_ASSERT(false);
...
bool Field_document::is_null(my_ptrdiff_t row_offset) const{
  ...
  fbson::FbsonValue *p =
    const_cast<Field_document*>(this)->get_fbson_value();
    ...
    Field_document(*document)
{
  ...
  for(Document_key *p; (p=it++);)
  {
    document_key_path.push_back(p, mem_root);
  }
  DBUG_ASSERT(inner_field);
    ...
    Field_document(*document)
{
  ...
  inner_field = document;
  document_key_path.empty();
  append_key_path(document_key_path, doc_path, skip_num);
  DBUG_ASSERT(inner_field);
  ...
  // Maybe we have an index for it!!
  bool found_doc_idx = false;
  if(document->real_field()->table->s->document_key_trie)
  {
    if (table->s->document_key_trie)
    {
      Document_key_trie* trie = table->s->document_key_trie;
      trie = trie->get(real_field()->field_name);

      /* find the document key in the trie that matches the path
       */
      Document_path_iterator it(this);
...
{
  /*
    It's a document field wrapping of another. So we need to get the data
    defined in the inner field first.
  */
  if(nullptr != get_inner_field())
  {
    // Get the data from inner field.
    fbson::FbsonValue *pval = get_inner_field()->get_fbson_value();
    // Get the data defined in the document path.
    return json_extract_fbsonvalue(&document_key_path, pval);
  }

  // It's the inner most field, where the data is stored in.
  DBUG_ASSERT(this == real_field());
  if(Field_blob::is_null())
    return nullptr;
  ...
  if (blob &&
      (pval = fbson::FbsonDocument::createValue(blob, get_length(ptr)))){
    return json_extract_fbsonvalue(&document_key_path, pval);
  }
  return nullptr;
}
...
Field *Field_enum::new_field(MEM_ROOT *root,
                             TABLE *new_table,
                             bool keep_type,
                             bool from_alias)
{
  Field_enum *res= (Field_enum*) Field::new_field(root,
                                                  new_table,
                                                  keep_type,
                                                  from_alias);
  if (res)
