...
void KEY_PART_INFO::init_from_field(Field *fld)
{
  ...
  /* A key part should never be initialized from a
  ...
  */
  ...
  case MDL_wait::TIMEOUT:
    my_error(ER_LOCK_WAIT_TIMEOUT, MYF(0),
             timeout_message("table closing", db.str,
                             table_name.str).c_ptr_safe());
    return TRUE;
...
/*
  ...
  execution).
 */
void TABLE::mark_columns_per_binlog_row_image(bool is_insert)
{
...
{
  mark_columns_per_binlog_row_image(true);
  if (triggers)
  {
    /*
